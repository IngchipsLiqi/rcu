...
static int8_t keyscan_bsp_check_state_para(const KEYSCAN_SetStateStruct* keyscan_set)
{
    if (keyscan_set == 0) {
        return -1;
    }
...
void keyscan_bsp_enable_wakeup_before_sleep(const KEYSCAN_SetStateStruct* keyscan_set)
{
    ...
    for (i = 0; i < keyscan_set->row_num; i++) {
        ...
        PINCTRL_Pull(keyscan_set->row[i].gpio, PINCTRL_PULL_DISABLE);
...
static key_pressed_callback_t key_pressed_callback = 0;
static uint32_t group0_input_map = 0;
static uint32_t group1_input_map = 0;
static uint32_t gpio_cb_isr(void *user_data)
{
    uint64_t current = GIO_ReadAll();
    uint32_t current_group0 = current & 0x1FFFFF;
    uint32_t current_group1 = (current >> 21) & 0x1FFFFF;
    if (current_group0 & group0_input_map) {
        key_pressed_callback();
    } else if (current_group1 & group1_input_map) {
        key_pressed_callback();
    } else {
    }
...
#define GPIO_GROUP0_END GIO_GPIO_20
#define GPIO_GROUP1_START (GPIO_GROUP0_END + 1)
void keyscan_bsp_enable_press_interrupt(const KEYSCAN_SetStateStruct* keyscan_set, key_pressed_callback_t fun)
{
    int i;
    uint8_t enable_gpio_group_0 = 0;
    uint8_t enable_gpio_group_1 = 0;
    ...
    for (i = 0; i < keyscan_set->row_num; i++) {
        ...
        GIO_SetDirection(keyscan_set->row[i].gpio, GIO_DIR_OUTPUT);
        GIO_WriteValue(keyscan_set->row[i].gpio, 1);
        if (gpio_retention_group[keyscan_set->row[i].gpio] <= GPIO_GROUP0_END) {
            enable_gpio_group_0 = 1;
        } else {
            enable_gpio_group_1 = 1;
        }
    }
    ...
    for (i = 0; i < keyscan_set->col_num; i++) {
        ...
        GIO_SetDirection(keyscan_set->col[i].gpio, GIO_DIR_INPUT);
        GIO_ConfigIntSource(keyscan_set->col[i].gpio, GIO_INT_EN_LOGIC_HIGH_OR_RISING_EDGE, GIO_INT_EDGE);
        if (keyscan_set->col[i].gpio <= GPIO_GROUP0_END) {
            enable_gpio_group_0 = 1;
            group0_input_map |= (0x1 << keyscan_set->col[i].gpio);
        } else {
            enable_gpio_group_1 = 1;
            group1_input_map |= (0x1 << (keyscan_set->col[i].gpio - GPIO_GROUP1_START));
        }
    }

    if (enable_gpio_group_0) {
        SYSCTRL_ClearClkGateMulti((1 << SYSCTRL_ClkGate_APB_GPIO0));
    }

    if (enable_gpio_group_1) {
        SYSCTRL_ClearClkGateMulti((1 << SYSCTRL_ClkGate_APB_GPIO1));
    }

    if (group0_input_map != 0) {
        platform_set_irq_callback(PLATFORM_CB_IRQ_GPIO0, gpio_cb_isr, 0);
    }

    if (group1_input_map != 0) {
        platform_set_irq_callback(PLATFORM_CB_IRQ_GPIO1, gpio_cb_isr, 0);
    }
